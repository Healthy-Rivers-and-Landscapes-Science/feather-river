...
```{r picture, echo = F, fig.cap = "Figure 1. Diagram of the data lifecycle highlighting the focus areas of this project", out.width = '75%', align = "center"}
...
Complete metadata are available on the [Environmental Data Initiative](https://portal.edirepository.org/nis/mapbrowse?packageid=edi.1705.2). Below are data definitions relevant to focus of this dashboard.

- **Location** refers to the area in the river that was snorkeled. This is the most coarse spatial category.
- **Location Table ID** refers to the specific section within the location that was snorkeled representing 25 meters by 4 meters. Locations have names and unique 
- **Transect Code**
...
There are two possible ways to filter the data: 1) `Attribute Filter` will allow user to filter by a specific location and location table Id; 2) `Map Filter` allows user to select a point on the map to view the the associated heatmap. Each option displays a heatmap of percent cover by type for each microhabitat unit. When a fish was present during the sampling, it is shown in a dark red outline. 
