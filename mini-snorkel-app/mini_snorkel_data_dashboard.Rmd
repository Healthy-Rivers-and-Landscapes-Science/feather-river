...
selectInput("location_filter", label = "Filter by Location:",
            choices = c(sort(unique(locations_sf$location))), selected = "hatchery ditch")
...
```{r}
# Note - discuss if this is the way we want to do the location ID filter. Considering if we need it
...
leaflet(locations_sf) |> 
  ...
  addCircleMarkers(color = ~colors_full,
                   radius = 6,
                   fill = TRUE, 
                   fillOpacity = 0.2,
                   opacity = 0.6,
