...
```{r, include=FALSE}
# redd survey points
redd_data <- read.csv(here::here("edi-redd", "data-raw", "combined_raw_data.csv"))

# surveying sites from gis data map
gdb_path <- here::here("edi-redd", "data-raw", "shapefiles", "b105_redd_survey_locations_maps_2021.gdb")
...
fixed_zeros <- read.csv(here::here("edi-redd", "data-raw", "qc-processing-files", "Faulty_lat_long_records_edits_Chris_111224.csv")) |> # csv with Chris' notes
  select(-1) |> 
...
fixed_off_channel <- read.csv(here::here("edi-redd", "data-raw", "qc-processing-files", "Off_channel_records_edit_Chris_110524.csv")) |> 
  glimpse()
...
reference_locations <- c(
  ...
  "Developing", "GreatWestern", "Palm", "TopofHatchery", "G95main", "LowerVanceEast", "bigriffle"
)
...
redd_observations_clean_1 <- redd_locations_fixed |> 
  mutate(location = case_when(
    ...
    location == "bigbar" | location ==	"bigbarriffle" ~	"bigbar",
...
gdb_path <- "edi-redd/data-raw/shapefiles/b105_redd_survey_locations_maps_2021.gdb"
layers <- st_layers(gdb_path)$name
...
write.csv(redd_observations_wnames, "edi-redd/data-raw/qc-processing-files/redd_observations_survey_wk.csv", row.names = FALSE)

# save clean data
write.csv(redd_observations, "edi-redd/data/redd_observations.csv", row.names = FALSE)
```


```{r, include=FALSE}
read.csv("edi-redd/data/redd_observations.csv") |> glimpse()
```
