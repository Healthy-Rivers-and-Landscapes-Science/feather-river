...
all_fish_data <- microhabitat_2002 |>
  ...
  left_join(
    weather_code_lookup_2002,
    by = c("Weather" = "WeatherCodeLookUpID")
  ) |> # all codes are in the lookup
  ...
  left_join(
    cgu_code_lookup_2002 |> mutate(CGUCode = tolower(CGUCode)),
    by = c("cgu" = "CGUCode")
  ) |>
  select(
    -cgu,
...
all_fish_data %>%
  ...
  theme_minimal() +
  labs(title = "Count over Time", y = "Number of Fish Observations")
```
...
all_fish_data %>%
  ...
  theme_minimal() +
  labs(title = "Dates that River miles Surveyed", y = "")
```
...
all_fish_data |>
  ...
  pivot_longer(
    cols = percent_fine_substrate:percent_boulder_substrate,
...
all_fish_data |>
  ...
  pivot_longer(
    cols = percent_no_cover_inchannel:percent_submerged_aquatic_veg_inchannel,
    names_to = "inchannel_cover_type",
    values_to = "percent"
  ) |>
...
all_fish_data |>
  ...
  pivot_longer(
    cols = percent_undercut_bank:percent_cover_more_than_half_meter_overhead,
    names_to = "overhead_cover_type",
    values_to = "percent"
  ) |>
...
all_fish_data |>
  ...
  theme_minimal() +
  labs(title = "Surface turbulence distribution") +
  theme(
    text = element_text(size = 18),
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)
  )
...
```{r}
nrow(all_fish_data |> filter(count > 0)) ==
  length(unique(all_fish_data$fish_data_id))
```
...
There are `r is.na(all_fish_data$channel_width) |> sum()` NA values
...
all_fish_data <- all_fish_data |>
  mutate(
    location = case_when(
      location %in% c("aleck riffle", "alec riffle") ~ "aleck riffle",
...
-   split out species and wild/hatchery
-   other?
...
microhabitat_with_fish_detections <- all_fish_data |>
  rename(
    transect_code = t_code,
    location_table_id = p_dat_id,
    surface_turbulence = sur_turb
  ) |>
  mutate(
    species = tolower(species),
    origin = case_when(
      species == "steelhead trout (wild)" ~ "wild",
      species == "steelhead trout, (clipped)" ~ "hatchery",
      T ~ NA
    ),
    species = case_when(
      species == "sacramento squawfish" ~ "sacramento pikeminnow",
      grepl("steelhead trout", species) ~ "steelhead trout",
      T ~ species
    ),
    count = ifelse(is.na(count), 0, count),
    channel_geomorphic_unit = tolower(channel_geomorphic_unit),
    channel_geomorphic_unit = case_when(
      channel_geomorphic_unit == "glide edgewater" ~ "glide margin",
...
survey_locations <- all_fish_data |>
  rename(transect_code = t_code, location_table_id = p_dat_id) |>
  mutate(
    weather = tolower(weather),
    channel_type = tolower(channel_type),
    location_revised = case_when(
      grepl("big hole", location) ~ "big hole",
      grepl("auditorium", location) ~ "auditorium riffle",
      location %in%
        c(
          "bedrock",
          "bedrock park",
          "bedrock park, unit #50"
        ) ~ "bedrock riffle",
...
survey_locations_2001 <- read_csv(here::here(
  "edi-mini-snorkel",
  "data",
  "archive",
  "survey_locations.csv"
))
...
setdiff(
  ...
  sort(unique(survey_locations_2001$location))
)
...
location_data_updated <- survey_locations |>
  ...
  mutate(
    latitude = case_when(
      location == "cox riffle" ~ 39.33564114789087, # based off of gps_coordinates
      location == "gateway riffle" ~ 39.45696577928121,
      (location == "shallow riffle" &
        channel_type == "mainchannel branch") ~ 39.32967570030937,
      (location == "shallow riffle" &
        channel_type == "mainchannel") ~ 39.33154115157018,
      T ~ latitude
    ),
    longitude = case_when(
      location == "cox riffle" ~ -121.63215000396984, # based off of gps_coordinates
      location == "gateway riffle" ~ -121.62560364814165,
      (location == "shallow riffle" &
        channel_type == "mainchannel branch") ~ -121.6285435907359,
      (location == "shallow riffle" &
        channel_type == "mainchannel") ~ -121.63033927513442,
      T ~ longitude
    ),
    coordinate_method = case_when(
      location %in%
        c(
          "cox riffle",
          "gateway riffle"
        ) ~ "assigned based on similar location",
      (location == "shallow riffle" &
        channel_type %in%
          c(
            "mainchannel branch",
    ...
    channel_location = case_when(
      location == "shallow riffle" ~ "HFC",
    ...
    channel_type = case_when(
      location == "trailer park" ~ "mainchannel",
      T ~ channel_type
    )
  ) # this removes 1 NA but is consistent with all other trailer parks
...
write_csv(location_data_updated, here::here("edi-mini-snorkel", "data", "survey_locations.csv"))
write_csv(
  microhabitat_with_fish_detections,
  here::here("edi-mini-snorkel","data", "microhabitat_observations.csv")
)
